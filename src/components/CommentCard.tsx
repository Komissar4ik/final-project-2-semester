...
export default function CommentCard({ comment, index = 0 }: CommentCardProps) {
  const { allUsers, currentUser } = useAppStore();
  const author =
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-1.5 px-2">
          <button className="flex items-center gap-1 text-xs text-stone-400 hover:text-rose-500 dark:text-white/30 dark:hover:text-rose-400 transition-colors group/like">
            <Heart size={12} className="group-hover/like:fill-rose-500 transition-colors" />
            {comment.likesCount > 0 && comment.likesCount}
          </button>
          <button className="text-xs text-stone-400 hover:text-stone-700 dark:text-white/30 dark:hover:text-white/60 transition-colors">
            Reply
          </button>
        </div>
      </div>
